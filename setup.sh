...
# Run this once on your Ubuntu sandbox VM inside the cloned repo folder
...
# -----------------------------------------------------------------------------
header "Credentials"
echo "You will need your Anthropic API key and GitHub PAT token."
echo ""

read -p "Enter your Anthropic API key: " ANTHROPIC_KEY
if [ -z "$ANTHROPIC_KEY" ]; then error "Anthropic API key cannot be empty."; fi
...
read -p "Enter your GitHub username for the bot account (e.g. zqproj): " GITHUB_USER
...
if ! grep -q "^workspace/\*\.db$" .gitignore 2>/dev/null; then
    echo "workspace/*.db" >> .gitignore
fi
...
- Ensure agents branch from dev
- Only merge to dev after QC passes
EOF
...
# -----------------------------------------------------------------------------
# Write Dockerfile (shared base for all agents)
# -----------------------------------------------------------------------------
...
git config user.name "zqproj"
git config user.email "zqproj@users.noreply.github.com"

# Set up credential store with PAT
...
git add .
git commit -m "chore: initial agent project structure [setup]" || warn "Nothing to commit or already committed."
...
echo "  cat workspace/status.json"
echo ""
echo "To check tickets:"
echo "  ls workspace/tickets/"
echo ""
